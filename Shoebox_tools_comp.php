<?php
// filename  Shoebox_tools_comp.php
// purpose   collect php functions here 
// usage     include "Shoebox_tools_comp.php";
// history
...
function mk_comp_array($id,$name,$addr1,$addr2,$addr3,$phone1,$phone2,$fax,$ctype,$acct,$notes,$bool) {
  ...
  if ($bool == 1) {
    if (array_key_exists($id,$comp_name)) {
      $sql = "update Shoebox.companies set active=1 where id=".$id;
    } else {
      $sql  = "insert into Shoebox.companies ";
...
function get_companies() {
  ...
  $sql .= " from Shoebox.companies where active=1 order by name";
...
function get_1_company($id) {
  ...
  $sql .= " from Shoebox.companies where id=".$id;
...
function upd_comp($id,$name,$addr1,$addr2,$addr3,$phone1,$phone2,$fax,$ctype,$notes) {
  global $version,$pdo;
  $sql  = "update Shoebox.companies set ";
  $sql .= " name='".$name."'" ;
  $sql .= ",addr1='".$addr1."'";
  $sql .= ",addr2='".$addr2."'";
  $sql .= ",addr3='".$addr3."'";
...
function del_comp($id) {
  ...
  if (array_key_exists($id,$inv_cid)) {
    $sql = "update Shoebox.companies set active=0 where id=".$id;
    $comp_active[$id] = 0;
  } else {
    $sql = "delete from Shoebox.companies where id=".$id;
    unset($comp_name[$id]);
    unset($comp_addr1[$id]);
    unset($comp_addr2[$id]);
    unset($comp_addr3[$id]);
...
function get_comp_list() {
  ...
  while (list($id, $name) = each($comp_name)) {
    if ($comp_active == 0) {
      $pre  = "<font color=blue>";
      $post = "</font>";
    } else {
