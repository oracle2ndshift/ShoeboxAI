<?php
// filename  Shoebox_tools_rpt.php
// purpose   collect php functions here
// usage     include "Shoebox_tools_rpt.php";
// history
...
function rpt_exec($schedE,$bdate,$edate,$vendor,$customer,$rtype,$exec_preview) {
  ...
  if ($rtype == 2 || $rtype == 3) {
    ...
    $sql .= "from Shoebox.companies where 1=1 ".$where." order by 2";
    ...
    $sql .= " left join Shoebox.companies c on i.cid=c.id ";
    $sql .= " left join Shoebox.pay p on i.id=p.iid ";
    ...
    // build the sql
    //  Sample:
    //    select id,name,Shoebox.acct_get_sum(id,'2015-1-1','2015-12-31','Hoff') 
    //    from Shoebox.acct;
    //
    ...
    $sql .= " a.atype     type,";
    $sql .= " a.level     level,";
    $sql .= " Shoebox.acct_get_sum(a.id,'".$bdate."','".$edate."','".$schedE."') amt ";
