<?php
// filename  Shoebox_tools_inv.php
// purpose   collect php functions here 
// usage     include "Shoebox_tools_inv.php";
// history
...
function get_open_invoices($itype,$schedE) {
  ...
  $sql .= " from Shoebox.inv i, Shoebox.companies c ";
...
function invpay_submit($itype,$invnum) {
  ...
  for ($i = 1; $i <= $invnum; $i++) {
    ...
    if ($amt != 0) {
      $sql  = "update Shoebox.inv set balance=balance-".$amt." where id=".$id;
      runsql($sql);
      $sql  = "insert into Shoebox.pay (iid,pdate,ptype,amt) values (";
...
function insinv($entity_id,$cid,$idate,$itype,$amt,$acct,$auto) {
  ...
  $sql  = "update Shoebox.companies set acct=".$acct." where id=".$cid;
  runsql($sql);
...
//
function delinv($id) { 
  runsql("delete from Shoebox.inv where id='".$id."'");
}
